...
if (session.getAttribute("allPizzaCache")==null)
	{
		...
		Pizza pizzaSave = (Pizza)(session.getAttribute("save"));
		session.setAttribute("allPizzaCache",pizzaPerm);
		if (pizzaSave!=null)
		{
			System.out.println("pizza noua "+pizzaSave.getName());
			pizzaPerm.add(pizzaSave);
		}
	}
else
	{
...
if (request.getParameter("act").equals("save"))
{
	...
	{		
		...
		else
		{
			...
			pizza.setDescription("pizza custom");
			...
			{
				System.out.println("adaug pizza "+pizza.getName());
				System.out.println("Pizza cache "+(ArrayList<Pizza>)session.getAttribute("allPizzaCache"));
				p.add(pizza);
				...
				Iterator<Ingredient> ing = pizza.getIngredients().iterator();
				System.out.println("ingred "+ pizza.getIngredients().size());
				System.out.println("ingredientele:" +pizza.getIngredients());
				System.out.println("null? = "+ing);
				
				if (null != ing)
				{
				while (ing.hasNext())
				{
					Ingredient ingred = ing.next();
					System.out.println(ingred.getId());
					ingredIds.add(ingred.getId());
	...
	else
	{
		...
		{
			System.out.println("adaug pizza cand save != null "+pizza.getName());
			p.add(pizza);
			((ArrayList<Pizza>)session.getAttribute("allPizzaCache")).add(pizza);
			session.setAttribute("save",p);
			session.setAttribute("Ingreds",null);
			List<Integer> ingredIds = new ArrayList<Integer>();
			Iterator<Ingredient> ing = pizza.getIngredients().iterator();
			System.out.println("ingred save sunt "+pizza.getIngredients().size());
			int i=0;
			while (ing.hasNext())
			{
				ingredIds.add(ing.next().getId());
				i++;
			}
			DBImplementation.getInstance(getServletContext()).addPizza(pizza, ingredIds);
			out.print("Pizza a fost salvata.");
