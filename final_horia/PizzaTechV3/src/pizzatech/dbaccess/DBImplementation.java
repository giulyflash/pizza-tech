...
public class DBImplementation implements DBInterface {
	...
	public List<Order> getAllOrders(int userId) {
		Query q = em.createNamedQuery(QueryConstants.GET_USER_ORDERS);
	...
	public int login(String username, String password) {
		Query q = em.createNamedQuery(QueryConstants.GET_USER);
		q.setParameter("username", username);
		q.setParameter("password", password);
		List<User> regs = (List<User>) q.getResultList();
		if (regs.isEmpty() == false) {
			return regs.get(0).getId();
		}
		return -1;
	}
